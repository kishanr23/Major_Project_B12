...
// Mock of Meshtastic's internal module base for Phase 1 testing
// In reality, this would inherit from `ProtobufModule` or similar.
...
class TrailGuardModule {
public:
    TrailGuardModule();
    ~TrailGuardModule() = default;

    // Called when a packet arrives on our specific portnum
    bool onReceive(const std::vector<uint8_t>& raw_bytes);
...
private:
    ...
    // Sized according to worst-case LoRa channel capacity over a 10-minute acceptance window.
    ...
    // Per-type payload size caps (bytes, serialized protobuf, excluding signature).
    // Derived from worst-case wire-format analysis; see docs/ARCHITECTURE.md.
    //   CheckIn worst case:   128 bytes (36-UUID hiker_id + 16 node_id + 5 ts + 66 sig)
    //   SOS worst case:       228 bytes (above + 9 lat + 9 lon + 82 message)
    //   TrailMessage cap:     140 bytes (free_text hard-capped at 140 chars in proto)
